...
set HMtag_map(hmstart) {
	family helvetica  weight medium   style r   size 10
...
proc HMtag_title {win param text} {
	upvar $text data
	wm title [winfo toplevel $win] "tkgoodstuff help: $data"
	set data ""
}
...
proc setup {} {
	...
	label .msg -textvariable message
        frame .middle
	scrollbar .yscrollbar  -command ".text yview"  -orient v
	scrollbar .xscrollbar  -command ".text xview"  -orient h
	option add *Text.height 40 startup
	option add *Text.width 100 startup
	text .text  -yscrollcommand ".yscrollbar set"\
	    -xscrollcommand ".xscrollbar set" \
	    -padx 3 -pady 3 -takefocus 0
	pack .top .msg .middle -side top -fill x
        pack  .middle -side top -fill both -expand 1
	pack .yscrollbar -side left -expand 0 -fill y -in .middle
	pack .text -side left -fill both -expand 1 -in .middle
	pack .status .quit -in .top -side left -expand y
	pack .xscrollbar -side top -expand 0 -fill x
...
set Size 3				;# font size adjustment
set Indent 1.2				;# tab spacing (cm)
set Home [pwd]/doc/index.html		;# home document
if [info exists argv(1)] {set Home [lindex $argv 0]}
