 ...
 *
 * SCCS: @(#) tkButton.h 1.4 96/12/13 15:13:11
 */
...
#define TkpDestroyTkgButton(butPtr) {}
...
typedef struct {
  ...
  char *iconside; 		/* Side of button occupied by image or bitmap
  ...
  int textWeight;             /* How much of any extra space goes to text 
				 * region */
  int imageWeight;            /* How much of any extra space goes to image
				 * region */
  Tk_Anchor textAnchor;		/* Where text should be displayed
				 * inside button region. */
  Tk_Anchor imageAnchor;	/* Where image or bitmap should be displayed
				 * inside button region. */
    ...
    int isDefault;		/* Non-zero means draw default ring. */
...
#define MENU_BUTTON_MASK	TK_CONFIG_USER_BIT << 2
#define CHECK_BUTTON_MASK	TK_CONFIG_USER_BIT << 3
#define RADIO_BUTTON_MASK	TK_CONFIG_USER_BIT << 4
#define ALL_MASK		(LABEL_MASK | BUTTON_MASK | MENU_BUTTON_MASK \
...
extern TkClassProcs tkpTkgButtonProcs;
extern Tk_ConfigSpec tkpTkgButtonConfigSpecs[];
...
EXTERN void		TkgButtonWorldChanged _ANSI_ARGS_((
			    ClientData instanceData));
EXTERN void		TkpComputeTkgButtonGeometry _ANSI_ARGS_((
			    TkgButton *butPtr));
EXTERN TkgButton *	TkpCreateTkgButton _ANSI_ARGS_((Tk_Window tkwin));
#ifndef TkpDestroyButton
EXTERN void 		TkpDestroyTkgButton _ANSI_ARGS_((TkgButton *butPtr));
#endif
#ifndef TkpDisplayButton
EXTERN void		TkpDisplayTkgButton _ANSI_ARGS_((ClientData clientData));
#endif
EXTERN int		TkInvokeTkgButton  _ANSI_ARGS_((TkgButton *butPtr));
